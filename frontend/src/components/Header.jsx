...
const Header = () => {
  ...
  useEffect(() => {
    const sendRequest = async () => {
      axios
        .get(
          "https://docs.google.com/spreadsheets/d/e/2PACX-1vTskbKwNeDFdZz_O3qhwTAfpvTdGIxb571uQ5wleV6H4BGY4h7tD-IEjhn-1fRC2XCHqGxRGOzb_x8j/pub?gid=0&single=true&output=csv"
        )
        .then((response) => {
          try {
    ...
    <div className='flex items-center justify-center h-[91.5vh] flex-col mr-[15rem] ml-[15rem] overflow-hidden'>
      ...
      {ticketsSold && (
        <div className='relative w-full rounded-xl pt-[0.1rem] pb-[0.1rem] bg-[#d9d9d9] mt-8'>
          a{" "}
          <div
            className={`absolute left-0 top-0 h-full items-center progress flex justify-end pr-5 rounded-l-xl text-white-100`}
            style={{
              width: `${ticketsSold > 1200 ? 100 : Math.floor((ticketsSold * 100) / 1200)}%`,
            }}
          >
            <h1 className='z-10 font-opensans'>
              {ticketsSold > 1200 ? 100 : Math.floor((ticketsSold * 100) / 1200)}%
            </h1>
          </div>
        </div>
      )}
      ...
      {ticketsSold && (
        <div className='flex justify-between w-full mt-14 font-opensans'>
          <div className='flex flex-col w-[15rem] text-blue-500 items-center font-semibold text-2xl border-4 border-blue-500 rounded-xl pr-8 pl-8 pt-2 pb-2 text-nowrap'>
            <h3 className='text-3xl'>{ticketsSold}</h3>
            <h4>Tickets Sold</h4>
          </div>
          <div className='flex flex-col w-[15rem] text-blue-500 items-center font-semibold text-2xl border-4 border-blue-500 rounded-xl pr-8 pl-8 pt-2 pb-2 text-nowrap'>
      ...
      <a className='bg-yellow-400 rounded-lg flex items-center justify-center text-center pr-12 pl-12 pt-2 pb-2 text-3xl font-bold font-opensans mt-10 cursor-pointer select-none hover:scale-105 transition-all duration-100'>
        Buy a ticket!
      </a>
    </div>
  );
